// src/componentes/ReportesTopInsumosCard.js
import React from "react";
import { View, Text, StyleSheet } from "react-native";

const ReportesTopInsumosCard = ({ topInsumos, accentColor }) => {
  if (!topInsumos || topInsumos.length === 0) return null;
      ...
      {topInsumos.map((i, idx) => (
        <View key={i.id} style={styles.rowItem}>
          <View style={styles.rowLeft}>
            ...
            >
              <Text style={[styles.indexCircleText, { color: accentColor }]}>
                {idx + 1}
              </Text>
            </View>
...
export default ReportesTopInsumosCard;
